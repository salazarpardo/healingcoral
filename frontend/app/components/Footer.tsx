...
function ColumnLinks({
  ...
  return (
    <div>
      {title && (
        <p className="text-navy font-bold text-xs uppercase tracking-wide mb-4">
          {title}
        </p>
      )}
...
export default async function Footer() {
  ...
  const copyright =
    s?.copyright ?? '© 2026 Healing Coral. All rights reserved.';
  ...
  return (
    <footer className="bg-teal text-white relative overflow-hidden">
      <div className="container pt-20 pb-8 px-4 sm:px-6">
          ...
          <Link href="/" className="flex items-center gap-2">
            {logo?.asset ? (
              <div className="relative w-30 h-24 shrink-0">
                <Image
                  src={urlForImage(logo).width(196).height(184).url()}
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    className="text-navy hover:text-teal-dark transition-colors p-1"
        ...
        <p className="text-navy text-xs pt-6">{copyright}</p>
      </div>
    </footer>
  );
}

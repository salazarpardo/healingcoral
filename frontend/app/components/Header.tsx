...
function HeaderCtaButton({
  ...
  const href = button.link
    ? linkResolver(button.link as DereferencedLink)
    : null;
  if (!href)
    return (
...
export default async function Header() {
  ...
  const mainNav =
    (
      settings as {
        mainNav?: Array<{ label?: string; link?: DereferencedLink }>;
      }
    )?.mainNav ?? [];
  const headerCta = (
    settings as { headerCta?: { buttonText?: string; link?: DereferencedLink } }
  )?.headerCta;
  const logo = (
    settings as { logo?: { asset?: { _ref: string }; alt?: string } }
  )?.logo;
      ...
      <header className="top-0 py-6 max-w-6xl mx-auto">
        <div className="container flex items-center justify-between h-20 px-4 sm:px-6 sm:py-6">
          <Link href="/" className="flex items-center gap-2">
            {logo?.asset ? (
              <div className="relative w-20 h-18">
                <Image
                  src={urlForImage(logo).width(180).height(168).url()}
          ...
          <MobileNav
            ...
            ctaHref={
              headerCta?.link
                ? linkResolver(headerCta.link as DereferencedLink)
                : null
            }
            logoUrl={
              logo?.asset
                ? urlForImage(logo).width(180).height(160).url()
                : null
            }
